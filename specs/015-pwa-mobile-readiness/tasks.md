...
- [X] T054 [P] [US5] **Note**: the `chromium` project has no explicit `testMatch`, so it inherits the top-level glob and will also run the Phase 15 mobile specs on a desktop viewport — guard mobile-only assertions by project name or viewport width so they do not fail on `chromium`, where the bottom nav is hidden. Write `apps/web/e2e/mobile-navigation.spec.ts` covering contract `mobile-interaction.md` checks MI-1 through MI-4 in both locales: the bottom nav renders role-permitted destinations plus "more" with the desktop sidebar hidden, the active destination carries `aria-current="page"` and a visible indicator, Arabic mirrors item order and drawer side relative to English, and a Viewer sees no forbidden destination in bar or drawer. **Review note**: `MobileNavDrawer` wraps the shared, centered `Dialog` rather than a slide-in side sheet, so there is no literal "open side" to assert for MI-3 — asserted instead that the bottom nav's visual order (bounding-box x-centers) reverses under RTL per the logical-CSS mechanism (N-5), and that the drawer's remaining destinations render identically in both locales. Also found and fixed a real gap while writing this: `BottomNav`'s landmark `aria-label` was hardcoded English ("Mobile navigation") regardless of locale — added a translated `nav.bottomNav` key and a `navLabel` prop (T057).
- [X] T055 [P] [US5] Add checks MI-5, MI-6, MI-10, and MI-11 to `apps/web/e2e/mobile-navigation.spec.ts`: every interactive control on the audited mobile screens has a bounding box of at least 44×44px, the bottom nav does not occlude the last list row or the primary action, all eight core tasks complete on a mobile viewport in both locales, and dialogs/drawers dismiss via control and back gesture without leaving the underlying screen. **Review note**: MI-5/MI-6 are scoped to the navigation surfaces this phase introduces (the bottom nav, its drawer, and one representative form screen) — a full app-wide touch-target audit is T062, not yet started. MI-6's first draft compared `boundingBox()` at the element's natural (pre-scroll) position, which says nothing about occlusion; fixed to `scrollIntoViewIfNeeded()` first. MI-11's first draft used `page.getByRole("button", { name: "Close dialog" })` unscoped, which matched both the dialog's close button and the backdrop overlay (also labelled "Close dialog") — fixed by scoping to the dialog. MI-10's first draft hung indefinitely on the second form (Incomes) because the amount field was queried immediately after a client-side nav click, before the new route's form had hydrated; fixed by waiting for the destination heading first.
- [X] T056 [P] [US5] Write Vitest specs for the bottom navigation in `apps/web/components/ui/__tests__/bottom-nav.test.tsx`: role filtering, the four-plus-"more" split, `aria-current` on the active item, keyboard focusability, and logical-property mirroring

### Implementation for User Story 5

- [X] T057 [US5] Implement `apps/web/components/ui/bottom-nav.tsx` per contract `mobile-interaction.md` §1: rendered below `lg` only, carrying the first four role-permitted destinations from the existing `navItems` array plus a "more" entry, using logical CSS properties for automatic RTL/LTR mirroring, `padding-block-end: env(safe-area-inset-bottom)`, `aria-current="page"` on the active item, and real focusable links. Already implemented in the working tree; verified via T054/T056 rather than rewritten, aside from the `navLabel` translation fix noted under T054.
- [X] T058 [US5] Render the bottom navigation from `apps/web/components/layout/WorkspaceShell.tsx`, reusing the existing role-filtered `navItems` array unchanged and routing the "more" entry to the existing `MobileNavDrawer` with the remaining destinations and quick actions (FR-030, FR-038). Already implemented in the working tree; verified via T054/T055.
- [X] T059 [US5] Add bottom padding equal to the bar height to the content region in `apps/web/components/ui/app-shell.tsx` so the bar never occludes the last row or a primary action (contract N-9). Already implemented in the working tree (`pb-24 md:p-6 lg:pb-6`); verified via MI-6 (T055).
- [X] T060 [US5] Apply `env(safe-area-inset-*)` on all four sides of the shell and to full-height overlays in `apps/web/components/ui/app-shell.tsx`, `apps/web/components/ui/dialog.tsx`, and `apps/web/components/ui/mobile-nav-drawer.tsx`, verified in portrait and landscape (contract S-2/S-3). **Review note**: already satisfied by the existing tree — `app-shell.tsx` carries `safe-area-block-start`/`-end` plus `safe-area-x` (physical, per S-2a) on the outer shell; `dialog.tsx` already applies explicit physical `padding-left`/`-right` plus top/bottom insets on its overlay container; `mobile-nav-drawer.tsx` wraps that same `Dialog`, so it inherits all four insets (S-3) rather than needing its own — its extra `pb-[env(safe-area-inset-bottom)]` is additional content clearance, not a missing inset. No code change made. Portrait/landscape is a physical-device concern this harness cannot exercise (no notch under Pixel-7 emulation, every `env()` inset resolves to 0) — left to the manual sweep (MI-12/T067).
- [X] T061 [US5] Convert full-height layouts from `vh` to `dvh` and make form footers sticky with safe-area padding in `apps/web/components/ui/app-shell.tsx` and the shared form layout, so the focused field and primary submit action stay visible with the on-screen keyboard open (FR-033, contract S-4/K-1/K-2). The `dvh` half was already done (`full-height-dvh` on the shell). The real work: added a `FormFooter` primitive to `apps/web/components/ui/form-field.tsx` (the shared form-primitives module) — `sticky bottom-24` (clearing the fixed bottom nav; matches the shell's `pb-24`) with `pb-[max(0.75rem,env(safe-area-inset-bottom))]`, reverting to `lg:static` where the nav is hidden — and applied it to the three keyboard-heavy entry forms named in the core-task inventory: `ExpenseForm.tsx`, `IncomeForm.tsx`, `ExtractionReviewForm.tsx`. Settings toggles and inline category rename were deliberately left alone (single-control, not keyboard-scroll forms). Verified: a new Vitest case asserts the sticky/safe-area classes; `mobile-navigation.spec.ts` MI-6 (nav clearance) and MI-10 (forms still submit) re-run green against a fresh build. The actual on-screen-keyboard reachability behavior (K-1/K-2) is **not** provable in this harness (headless has no real keyboard) and is left to the manual sweep (T067).
- [X] T062 [US5] Audit the mobile screens for the 44×44px touch-target minimum and adjacent-control spacing, adjusting the affected primitives in `apps/web/components/ui/` (FR-034, contract T-1/T-2). Shared primitives (`Button`/`IconButton`/`Input`/`Select`/`Textarea`, plus `dropdown-menu.tsx`, `tabs.tsx`, `pagination.tsx`, `file-upload.tsx`) were already `min-h-11`-compliant. The gap was raw, non-primitive controls that bypass them; fixed the highest-value one first — `apps/web/components/ui/error-state.tsx`'s retry button, a shared primitive reused across 7+ screens (category, expense/income history, AI settings, reports, files) — then swept the remainder: `LanguageSwitcher.tsx`, `PeriodSelector.tsx` (2 preset buttons, apply button, 2 date inputs), `AiSpendingSummary.tsx`, `AutoDeleteToggle.tsx` (wrapped the 20×20 checkbox in a padded `<label>` tap zone rather than inflating the checkbox itself), `WorkspaceSelector.tsx`'s retry link, `CategoryBreakdown.tsx`'s drilldown toggle, and the Dashboard's "Add income"/"Add expense" quick-action links (`h-10` → `min-h-11`). Sign-in/sign-up/reset-password were deliberately left out (pre-auth, outside this phase's "mobile screens," small inline sentence-links). The pre-existing uncommitted one-off `h-9`/`h-10` → `h-11` edits in `CategoryList.tsx`, `FileRow.tsx`, `ExpenseFileAttach.tsx`, and `WorkspaceCurrencySelector.tsx` predate this session; left as-is (not refactored into primitives, to avoid unnecessary churn) but are now legitimately part of T062's surface.
  **Review correction (found by re-running `visual-regression.spec.ts`)**: `WorkspaceSelector.tsx`'s "Create a team workspace" link was initially inflated to `min-h-11` too. That panel stacks several compact rows in the persistent per-page header, so the +24px pushed the *entire page* down, breaching the 1% diff threshold on `record-list-and-form` (confirmed via the diff image: a uniform downward shift from the sidebar down). Reverted to its natural size and documented as a deliberate exception (same reasoning as the sign-in/sign-up cross-links) rather than fixed with a layout-safe technique (e.g. an absolutely-positioned invisible hit-slop pseudo-element), since that would make the 44px target unverifiable by the same `boundingBox()` method used everywhere else in this suite. The corresponding e2e assertion was removed. Re-ran `visual-regression.spec.ts` after the revert: `record-list-and-form` now passes; the only remaining failure is the already-known `record-mobile-card` (T069, confirmed by its diff image — an Edit/Delete label wrapping difference, not a page-wide shift).
  **Verified**: new e2e case (`mobile-navigation.spec.ts`) checks all of the above on real rendered mobile viewports; T-1 (bounding box) is proven this way, but T-2 (adjacent-control spacing) has no dedicated assertion — inspected only (existing `gap-2`/`gap-3` utilities look adequate; the `LanguageSwitcher` segments are intentionally edge-to-edge, a standard segmented-control pattern). `reports-category-drilldown.spec.ts`, `category-management.spec.ts`, and `ai-settings.spec.ts` re-run green against these same edited shared primitives.
- [X] T063 [US5] Ensure dialogs, sheets, and drawers close on the device back gesture and their dismiss control without navigating away from the underlying screen, and return focus to the opening control, in `apps/web/components/ui/dialog.tsx` and `apps/web/components/ui/mobile-nav-drawer.tsx` (FR-035, contract T-3/T-4). Already implemented in the working tree (history-`pushState`/`popstate` handling for the back gesture, `previous?.focus()` restore on close) — no code change. `mobile-nav-drawer.tsx` inherits this by wrapping `Dialog`. Added `apps/web/components/ui/__tests__/dialog.test.tsx` (5 cases) as the one genuinely missing piece: focus-restore via the close button and via Escape (T-4), the back gesture closing the dialog (T-3), and that the compensating `history.back()` fires exactly once when closed by a control and not at all when already closed by the gesture itself. This is jsdom-level, not a real browser — `mobile-navigation.spec.ts`'s MI-11 already proves the T-3 behavior end-to-end through the real mobile nav drawer.
- [X] T064 [US5] Ensure in-progress form input survives an orientation change: confirm form state is held in React (via `react-hook-form`) rather than in layout-dependent DOM state across the income, expense, category, AI review, and settings forms, and add an orientation-change assertion (rotate the viewport mid-edit, confirm field values persist) to `apps/web/e2e/mobile-navigation.spec.ts` (FR-036, contract K-4). Verification, not a code fix: Expense/Income/Category forms use `react-hook-form`; the AI-review and AI-settings forms use plain `useState` — both satisfy K-4's actual requirement ("lives in React, never DOM state"); none of the five forms conditionally swaps or remounts inputs across breakpoints (layout changes are pure CSS Grid/flex reflow of the same DOM nodes), so nothing was at risk of dropping input. Added one e2e case that fills a field in each of the five forms, calls `page.setViewportSize()` to swap width/height (simulating the rotation Playwright cannot perform on a real device), and asserts the value is intact after and after rotating back. **Honestly**: this is a regression guard against a future viewport-conditional-remount regression, not proof current forms are keyboard/rotation-safe on a physical device — it passes because no code path in this codebase drops the value, which was already true before this task.
...
- [X] T065 [US6] Write `docs/mobile/app-store-readiness.md` with the sections required by research R-012: verified installability characteristics and platforms checked, the icon/asset inventory plus what a native shell additionally needs (the deferred iOS splash matrix and store icon sizes), the authentication and session rules a packaged client must preserve, the backend-financial-authority rule (no client calculation, no offline write), workspace-isolation requirements for any client-side cache, capture and upload constraints (file types, 10 MB, no client-side image processing), and an explicit Phase 16 deferral list. The document already existed on disk before this pass (not authored by this task run); treated as a first draft and verified against R-012 line-by-line rather than accepted on sight.
- [X] T066 [US6] Verify `docs/mobile/app-store-readiness.md` against the Phase 16 goals and exit criteria in `docs/implementation-plan.md`, confirming each prerequisite is recorded as satisfied or deferred and that the document proposes zero new endpoint, request/response change, schema change, or native artefact (SC-013). **Verification found and fixed three real gaps against R-012's own required section list**: (1) "platforms checked" was never actually stated — added an explicit line noting only Playwright's emulated Pixel 7 (`mobile-ltr`/`mobile-rtl`) and desktop Chromium have been exercised, and that no physical Android/iOS device has been checked (`quickstart.md`'s Manual sweep record / T067, Phase 9, confirmed empty — not run, and correctly not run here since Phase 9 is out of scope for this task); (2) R-012 explicitly requires "workspace-isolation requirements for any client-side cache" as a forward-looking requirement for a native shell, which was missing — the draft only described the *web* client's existing behaviour; added an explicit requirement (evict on switch, clear on sign-out/expiry, no cross-user leakage, citing FR-023/FR-024/FR-029); (3) Phase 16 goal "support core financial, file upload, AI review, report, and settings flows" had no corresponding recorded confirmation — added a line citing Phase 15's SC-006 (all eight core mobile tasks complete in both locales). Also tightened the asset-inventory wording to name "store icon sizes" explicitly, matching R-012's own phrasing. Cross-checked every Phase 16 goal and exit criterion in `docs/implementation-plan.md` against the document; the two criteria genuinely out of scope for a technical readiness record ("keep the application free", "no paid feature tier") are business/pricing constraints already enforced elsewhere (the constitution), not omissions. Confirmed zero new endpoints, request/response changes, schema changes, or native artefacts are proposed (SC-013). No dedicated automated test suite applies to a documentation-only change (no markdown linting configured in this repo); Vitest/Playwright/tsc are unaffected since no code file changed.
...
- [X] T069 **Known pre-existing failure**: `e2e/visual-regression.spec.ts` fails on the `record-mobile-card` snapshot (ar + en, `chromium` and `mobile-rtl`) with an identical pixel delta on a clean pre-Phase-15 tree — the committed Phase 14 baselines were captured in an environment whose font metrics wrap the Edit/Delete button labels differently. Diagnosed and re-run: the full pre-Phase-15 matrix produced 30 passing tests and 11 pre-configured environment skips; the only failures were the four expected `record-mobile-card` snapshots (Arabic/English × Chromium/mobile RTL, 1,765 / 1,864 differing pixels, 4% ratio). No snapshot was updated. The rerun exposed one real mobile-drawer navigation race: the dialog history sentinel could return a quick-action link to the underlying dashboard. Fixed it in `dialog.tsx` by removing the sentinel before calling the router, propagated the navigation callback through `mobile-nav-drawer.tsx`/`WorkspaceShell.tsx`, added a focused dialog unit regression test, and re-ran `refresh-responsive.spec.ts` successfully on Chromium and mobile RTL. This is an interaction fix; the screenshot mismatch remains an environment/baseline issue, not a Phase 15 visual regression (FR-044, SC-012)
- [X] T070 Run `npm run test:unit` in `apps/web` and confirm the full Vitest suite passes — 38 files / 131 tests passed on 2026-07-24
- [X] T071 Run the entire `pytest` suite in `apps/api` **unmodified** and confirm it passes, as the SC-011 gate proving no backend endpoint, schema, financial-calculation, or role-permission outcome changed — 180 passed in 544.87s on 2026-07-24
- [X] T072 Confirm the complete Phase 15 diff touches no file under `apps/api/` or `supabase/`, and that no out-of-scope capability (native packaging, push notifications, background sync, offline writes, payments, dark mode, global search, export) was introduced (FR-038, FR-040) — compared `main` through the current working tree: 91 changed paths, zero protected-path changes, no untracked files, and clean `git diff --check`; production changes are limited to `apps/web` PWA/mobile-readiness surfaces plus specifications/docs. The service worker has only install/activate/fetch/message handlers, and no native/capability file or implementation was found.
- [X] T073 Complete the post-phase comparison in the "Baseline Appendix" of `specs/015-pwa-mobile-readiness/quickstart.md` against the T001 baseline, confirming no significant regression in initial load or interaction responsiveness (FR-045, SC-014) — fresh production-build comparison on 2026-07-24: `refresh-responsive`, `pwa-installability`, and `cache-isolation` passed across Chromium, mobile RTL, and mobile LTR (26 tests total). The baseline and conclusion are explicitly qualitative; real-device checks remain outside this task.
