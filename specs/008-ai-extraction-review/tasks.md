...
- [X] T032 [P] Backend **dedicated key-secrecy** test `apps/api/tests/test_extraction_secrecy.py`: the decrypted BYOK key never appears in any trigger/confirm/discard response, error, or captured log line; `get_workspace_ai_key_for_extraction` is the only function that ever calls `vault.decrypted_secrets` (constitution VI/XIV NON-NEGOTIABLE; research Decisions 1–3)
- [X] T033 [P] Backend test `apps/api/tests/test_extraction_authorization.py`: full role matrix across trigger/list/get/confirm/discard — Viewer view-only everywhere; Member own-record only for confirm/discard/edit; Owner/Admin act on any extraction (SC-007)
- [X] T034 [P] Backend test `apps/api/tests/test_extraction_isolation.py`: every endpoint returns `404` for a workspace/file/extraction the caller is not a member of; anonymous requests → `401` (SC-008)
- [X] T035 [P] Complete and proofread en/ar i18n strings for all extraction surfaces and verify RTL layout of the review screen, pending-review queue, status badge, and discard dialog
- [X] T036 [P] Playwright e2e `apps/web/e2e/extraction.spec.ts`: trigger → review → confirm happy path; trigger → discard path; assert the decrypted key never appears in any network response body
- [X] T037 Run `quickstart.md` validation end to end (all `test_extraction_*` pytest suites + frontend tests/e2e + the manual role smoke matrix + the key-secrecy log grep) and record results — **Results (2026-07-05)**: full backend suite 93/93 passed; full frontend suite 47/47 passed; `extraction.spec.ts` e2e 2/2 passed (trigger→review→confirm, trigger→discard, both locales); key-secrecy `docker`/dev-server log grep for the configured key → 0 matches; `get_workspace_ai_key_for_extraction` function count → 1 (matches quickstart Scenario 2's exact check). All 9 quickstart.md scenarios have a passing automated equivalent (Scenario 5's Viewer role-matrix additionally spot-checked manually in-browser during Phase 3); none required manual re-verification beyond what's already covered by `test_extraction_authorization.py` + the e2e run.
- [X] T038 [P] Review `apps/api/app/services/extractions.py` + `ai_providers.py` + route error handling for consistent error codes and **no key/provider-body leakage** in logs/errors; confirm SQLAlchemy `echo` is off (reuses Phase 7 research Decision 7's discipline) — verified: `create_async_engine` has no `echo` param (defaults off); zero logging calls in either module; error codes cross-checked against `contracts/extraction-api.md`'s envelope table and found consistent (including `storage_unavailable`, reused verbatim from Phase 6's `services/files.py` for the same class of error)
- [X] T039 [P] Document the `ai_extractions` table, the two new RPCs, the provider REST call shapes, and the (unchanged, no new env vars) configuration in `supabase/README.md`
