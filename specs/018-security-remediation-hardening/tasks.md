...
- [x] T025 [US1] Author `specs/018-security-remediation-hardening/rollback.sql` per the contract's Reversal section, split into two independently runnable targets — `rollback_target=phase3` (reverse relocation) and `rollback_target=identity_guard` (reverse identity guard) — selected by an explicit required psql variable so the two changes can be reverted independently and can never be composed in one run; it MUST NOT be placed in `supabase/migrations/` (FR-013, FR-014)
...
- [x] T034 [US2] Extend `specs/018-security-remediation-hardening/rollback.sql`'s `identity_guard` target so reverting the guard restores the pre-guard body without reverting the relocation, and refuses to run (rather than creating the routine) when it is not already present in `private` (FR-014, MG-3)
- [x] T035 [US2] Run `apps/api/tests/test_signup_bootstrap.py` (RG-7) plus a manual end-to-end signup through the app per quickstart Step 4a, confirming a brand-new account gets its personal workspace (SC-007)
...
- [x] T085 Run the complete backend suite `cd apps/api && python -m pytest tests/ -q` and diff against `specs/018-security-remediation-hardening/baseline-pytest.txt`; confirm every pre-existing test passes with **no assertion modified**, and explicitly list any test that had to change with its justification (FR-039, SC-017)
- [x] T086 [P] Run `npm run test --workspace=@smart-expense/web` and the **CI-equivalent functional browser gate** `cd apps/web && npx playwright test e2e --workers=1 --grep-invert "design refresh visual regression"`; confirm no frontend regression from the new message key (FR-039, SC-019). The gate is deliberately split to mirror `.github/workflows/ci.yml`: visual regression is a **separate** job pinned to `mcr.microsoft.com/playwright:v1.61.1-noble` and is verified as quickstart **Step 9c** under T091, not as a task of its own — a bare `npx playwright test e2e` conflates them and reports Linux-baseline snapshot diffs as functional failures on any non-Linux host
- [x] T087 [P] Run **MG-1**: apply `supabase/migrations/20260731000000_private_schema_privileged_functions.sql` twice in succession and confirm the contract's verification queries return identical results (FR-012, SC-016)
- [x] T088 [P] Run **MG-2**: execute `specs/018-security-remediation-hardening/rollback.sql` with `-v rollback_target=phase3` against a **fresh disposable** database and confirm each of the four functions returns to `public` exactly once with no `private` copy remaining, with their original grants and metadata, with the Phase 4 identity guard retained, and with `handle_new_user` re-qualified to `public.ensure_personal_workspace`; verify the application gate against a **compatible public-calling application revision** — the current private-qualified tree is expected to fail against this target and must not be claimed compatible (FR-013, SC-016)
- [x] T089 [P] Run **MG-3**: confirm the `phase3` and `identity_guard` targets of `rollback.sql` can each be applied without the other, and that a missing target, an unknown target, `identity_guard` against a non-private routine, and an ambiguous public+private state each abort with a non-zero exit and no database change (FR-014)
- [x] T090 [P] Run **MG-4** and **MG-5** static gates: `grep -icE 'create policy|drop policy|create table|alter table'` on the migration returns 0, and grepping `apps/api/app` for `public.ensure_personal_workspace|public.find_user_profile_by_email|public.get_workspace_ai_key_for_extraction|public.shares_workspace_with` returns no matches (FR-004, FR-005, FR-010, SC-006)
- [x] T091 Execute quickstart Steps 1–9 from `specs/018-security-remediation-hardening/quickstart.md` and record each expected/actual result, including the EX-5 control check in Step 1 and the co-member visibility check in Step 4h that exercises the relocated `shares_workspace_with` through its RLS policy. Step 9 has three parts: 9a (backend + web unit), 9b (functional e2e, visual regression excluded), and 9c (visual regression in the pinned `mcr.microsoft.com/playwright:v1.61.1-noble` container). **All three are green as of Correction Pass C** — 9c passed 4/4, exit 0, with the committed baselines left unregenerated. This covers Steps 1–9 only; the hosted release gates T092 and T093 remain open and are not part of this task
- [x] T092 **RELEASE GATE — cannot be automated**: execute quickstart Step 10; confirm the target deployment environment's exposed-schema list is `public, graphql_public` and excludes `private`; attach a screenshot or API-settings dump as evidence to the release record (FR-038, SC-018). **PASSED** — Dashboard → Integrations → Data API → Settings → Exposed schemas reads **"2 of 3 schemas exposed"** with `graphql_public` ✓ and `public` ✓ ticked and **`private` present in the list but NOT ticked**. Evidence: a hosted Dashboard screenshot of that panel, reviewed as external release evidence on 2026-08-03 and retained with the release record, not in this repository. Corroborated by T093, whose live `404 / PGRST202` is the runtime behaviour this setting produces
- [x] T093 **RELEASE GATE** — run the deployed smoke assertion from quickstart Step 10: `POST {DEPLOYED_URL}/rest/v1/rpc/ensure_personal_workspace` with a real user token returns `404`; this is the only check that catches hosted dashboard drift (FR-038, SC-018). **PASSED** against `https://wyno***.supabase.co` with a publishable key and an ordinary `role=authenticated` token: target returned **404 / PGRST202** ("Searched for the function `public.ensure_personal_workspace` with parameters `target_email, target_user_id` … no matches were found in the schema cache"), and the EX-5 control `clear_workspace_ai_key` returned **403 / 42501 `not_owner`** — non-404, so the harness is proven working. No `service_role` or secret key was used and nothing was mutated. **This closes T093 only — it is not evidence for T092**, which still requires the Dashboard exposed-schema list itself
- [x] T094 [P] Update `docs/deployment.md` with a Phase 18 operational section: the `private` schema exists and must never be added to exposed schemas; `APP_ENV` must be set to `production`; rate-limit allowances are per instance; `rollback.sql` location and usage
- [x] T095 [P] Add a short "Phase 18 security posture" note to `supabase/README.md` recording that privileged routines live in `private` and that `EXECUTE` must never be revoked from `authenticated` on the RLS helpers
...
- [x] T097 Record final phase status in `specs/018-security-remediation-hardening/spec.md`: change **Status** from `Draft` to `Implemented`, and state explicitly whether Phase 9 (User Story 7) was completed or dropped so the residual risk is not silently implied to be closed
