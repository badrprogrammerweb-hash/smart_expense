    ...
    <section className="rounded-[var(--radius-card)] border bg-card p-5 text-card-foreground shadow-[var(--shadow-card)]">
      ...
      {items.length === 0 ? (
        ...
        <div className="mt-4 grid gap-3 md:hidden">
          {items.map((item) => <MobileRecordCard key={item.user_id} title={item.display_name} fields={[{ label: t("recordCount", { count: item.records_created }), value: item.records_created }]} />)}
        </div>
      </>)}
    </section>
  );
