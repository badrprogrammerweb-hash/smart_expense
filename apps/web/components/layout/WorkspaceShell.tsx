...
import { getRoleLabel } from "@/lib/i18n/workspace-labels";
...
function WorkspaceFrame({ children }: { children: ReactNode }) {
  ...
  const mobileNavigation = <div className="space-y-6">{remainingNavigation}<div className="border-t pt-4"><div className="grid gap-2">{quickActions.map((action) => { const Icon = action.icon; return <Link className="inline-flex min-h-11 items-center gap-2 rounded-[var(--radius-control)] bg-secondary px-4 text-sm font-medium text-secondary-foreground no-underline hover:bg-[var(--color-surface-hover)]" href={action.href} key={action.href}><Icon className="size-4" aria-hidden="true" />{action.label}</Link>; })}</div></div></div>;

  return <AppShell bottomNav={<BottomNav items={bottomNavItems} moreLabel={t("more")} navLabel={t("bottomNav")} onMore={() => setMobileNavigationOpen(true)} />} connectivity={<OfflineBanner />} staleNotice={<><IndeterminateOutcomeNotice /><StaleDataNotice /></>} sidebar={<div className="hidden lg:block">{navigation}</div>} header={<TopHeader title={t("workspaceRole", { role: getRoleLabel(t, role) })} actions={<><Button className="lg:hidden" variant="ghost" size="icon" aria-label={t("openNavigation")} onClick={() => setMobileNavigationOpen(true)}><Menu className="size-4" /></Button><WorkspaceSelector /><Button variant="secondary" onClick={() => void signOut()}><LogOut className="size-4" aria-hidden="true" />{t("signOut")}</Button><MobileNavDrawer open={mobileNavigationOpen} onOpenChange={setMobileNavigationOpen} onNavigate={router.push} title={t("mobileNavigation")}>{mobileNavigation}</MobileNavDrawer></>} />}><div className="mx-auto max-w-7xl" aria-disabled={!canMutate}>{children}</div></AppShell>;
}
